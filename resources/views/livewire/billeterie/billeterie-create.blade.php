<div class="card">
    <div class="card-header card-head">
        <h1 class="bg-card text-center text-white card-head"><i class="fa fa-book me-2"></i>Formulaire d'enregistrement des réçus</h1>
    </div>
    <div class="card-body">
            ...
            <div class="row mt-3">
                <div class="col-md-11 m-auto">
                    ...
                    <div class="row">
                        <div class="col-md-6 my-2">
                            <div class="form-floating">
                                <select class="form-select border-input @error('etudiant_id') is-invalid @enderror" wire:model="etudiant_id" id="etudiant_id" wire:click='clearStatus'>
                                    <option value="">Séléctionner un étudiant</option>
                                    @foreach ($etudiants as $etudiant)
                                        <option value="{{ $etudiant->id }}" wire:key="{{ $etudiant->id }}">{{ $etudiant->ine }} - {{ $etudiant->prenom }} {{ $etudiant->nom }}</option>
                                    @endforeach
                                </select>
                                <label for="floating" class="label-control label-text">Etudiants</label>
                        ...
                        <div class="col-md-6 my-2">
                            <div class="form-floating">
                                <select class="form-select border-input @error('promotion_id') is-invalid @enderror" wire:model="promotion_id" id="promotion_id" wire:click='clearStatus'>
                                    <option value="">Séléctionner une promotion</option>
                                    @foreach ($promotions as $promotion)
                                        <option value="{{ $promotion->id }}" wire:key="{{ $promotion->id }}">{{ $promotion->promotion }}</option>
                                    @endforeach
                                </select>
                                <label for="floating" class="label-control label-text">Prmotions</label>
                                <div class="invalid-feedback">@error('promotion_id') {{ $message }} @enderror</div>
                            </div>
                        </div>
                        <div class="col-md-6 my-2">
                            <div class="form-floating">
                                <select class="form-select border-input @error('nature_recu_id') is-invalid @enderror" wire:model="nature_recu_id" id="nature_recu_id" wire:click='clearStatus'>
                                    <option value="">Séléctionner un type de réçu</option>
                                    @foreach ($naturerecus as $naturerecu)
                                        <option value="{{ $naturerecu->id }}" wire:key="{{ $naturerecu->id }}">{{ $naturerecu->nature }}</option>
                                    @endforeach
                                </select>
                    ...
                    <div class="row">
                        <div class="col-md-6 my-2">
                            <div class="form-floating">
                                <select class="form-select border-input @error('modereglement') is-invalid @enderror" wire:model="modereglement" id="modereglement" wire:click='clearStatus'>
                                    <option value="">Séléctionner un mode réglement</option>
                                    <option value="Espece">Espece</option>
                                    <option value="Electronique">Electronique</option>
                                </select>
                                <label for="floating" class="label-control label-text">Mode de réglement</label>
                        ...
                        <div class="col-md-6 py-2">
                            <div class="d-flex flex-row justify-content-center gap-3">
                                <div>
                                    <button type="submit" class="btn-modal py-3"><i class="fa fa-check me-2"></i>Enregistrer un récu</button>
                                </div>
                                <div>
                                    <button type="button" wire:click='cancel' class="btn btn-danger py-3"><i class="fa fa-times me-2"></i>Annuler l'enregistrement</button>
                                </div>
                            </div>
                        </div>
                    </div>
