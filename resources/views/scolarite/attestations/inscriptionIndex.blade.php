...
<div class="container border border-5 border-primary my-5 text-dark">
        ...
        <div class="row text-center mb-2 ">
            @foreach ($attestations as $attestation)
                @foreach ($niveaux as $item)
                    @if ($item->id === $attestation->niveau_id && $attestation->attestation_id === $attestation->id)
                        @dump($item->niveau)
                    @endif
                @endforeach
                
            @endforeach
            <h5 class="text-uppercase fw-bold fs-4">Attestation d'attestation</h5>
            {{-- <h5 class="text-uppercase fw-bold">Attestation de reattestation</h5> --}}
        </div>
        <div class="row">
            <p class="contenu">
                Je soussigné 
                ...
                Matricule : <b>{{$attestation->etudiant->ine}} </b> a été insctit.e aux <b>Semestres S7 et S8</b>(4 <sup>ème</sup> Année) du Département <b>{{$attestation->etudiant->programme}}</b> au compte de l'année universitaire 
                <b>
                    @foreach ($annee_universitaires as $annee_universitaire)  
                        {{-- @if ($attestation->annee_universitaire_id === $annee_universitaire->id) --}}
                        {{$attestation->annee_universitaire->annee_universitaire}}   
                        {{-- @endif --}}
                        {{-- @dd($attestation->annee_universitaire) --}}
                    @endforeach
